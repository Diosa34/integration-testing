...
public class Sin extends FunctionWithValidation {
    public Sin() {
        super();
    }
    ...
    public BigDecimal calculate(BigDecimal x, BigDecimal precision) {
        ...
        do {
            prev = sum;
            sum = sum.add((((new BigDecimal(-1)).pow(n)).multiply(X.pow(2 * n + 1))).divide(this.getFactorial(2 * n + 1), DECIMAL128.getPrecision(), HALF_EVEN));
            n++;
